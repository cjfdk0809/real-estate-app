...
# Claude API에 전송할 프롬프트 (NPL 자산관리부 관점)
ANALYSIS_PROMPT = """당신은 키움에프앤아이 자산관리부의 NPL(부실채권) 권리분석 전문가입니다.
아래 등기부등본 텍스트를 분석하여 정확한 JSON 형식으로만 응답하세요.
...
4. 금액은 모두 원 단위 숫자 (예: 540,000,000원 → 540000000)
...
def analyze_registry_with_claude(registry_text, api_key):
    ...
    message = client.messages.create(
        model="claude-sonnet-4-6",  # 최신 Sonnet 사용 (2026.02 출시, 권리분석에 적합)
        max_tokens=8000,
        messages=[
            {
                "role": "user",
                "content": ANALYSIS_PROMPT.format(registry_text=registry_text)
            }
        ]
    )
    ...
    # 응답에서 JSON 부분만 추출 (혹시 ```json 으로 감싸진 경우 대비)
