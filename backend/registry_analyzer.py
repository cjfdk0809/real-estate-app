...
6. 근저당권이전·확정채권양도·계약(에 의한)양도·근저당권부질권 등 "기존 근저당권이 이전/양수된 부기등기"는 별도의 담보로 계상하지 말고, 원 근저당권 1건으로만 표시하세요. 이때 creditor_or_holder는 "현재(최종) 채권자"로, date는 원 근저당권 설정일(순위)로 기재합니다. 같은 채권최고액이 두 번 나오지 않게 하고, total_secured_debt에도 이전분을 중복 합산하지 마세요. (예: 구로신협 근저당 → 웰컴저축은행으로 이전 = 웰컴저축은행 근저당 1건)
