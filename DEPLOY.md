...
- ✅ **팀 공용 데이터** — 아래 "공용 공유 활성화"를 설정하면 누구나 URL 접속 시
  같은 본건·거래사례·추정가를 봄(시연회에서 전원이 동일 사례로 판단 가능).
  미설정 시에는 각 브라우저 localStorage 에만 저장됨.
...
각 팀원은:
- 자신의 PC/휴대폰에서 URL 접속
- **공용 공유 활성화 시**: 누구나 같은 본건·거래사례·추정가를 조회·수정 (시연용 공동 작업 공간)
- 국토부 자동수집 기능 공유 사용 (백엔드는 공통)

---

# 3.5단계: 공용 공유 활성화 (시연회 전 필수)

URL 접속만으로 **모든 사람이 같은 데이터(추정가 포함)**를 보게 하려면 두 가지가 필요합니다.
둘 중 하나라도 빠지면 데이터가 각 브라우저에만 저장되어 다른 PC/폰에서 안 보입니다.

## 3.5-1) Supabase 환경변수 등록 (Render 대시보드)

Render → 해당 서비스 → **Environment** → 아래 두 개 추가:

| Key | Value |
|---|---|
| `SUPABASE_URL` | `https://<프로젝트>.supabase.co` |
| `SUPABASE_KEY` | Supabase **service_role**(secret) 키 |

저장하면 Render가 자동 재배포됩니다.

## 3.5-2) user_states 테이블 생성 (Supabase SQL Editor)

Supabase → **SQL Editor** → New query → 저장소의
`migrations/002_user_states_schema.sql` 내용을 붙여넣고 **RUN**.

## 3.5-3) 확인

브라우저에서 `https://<배포주소>/api/health` 를 열어 아래를 확인:

```json
"supabase": { "connected": true, "url_set": true, "key_set": true }
```

`connected: true` 면 공유 준비 완료. 한 PC에서 본건을 열어 추정가를 산출한 뒤,
다른 기기(폰)에서 같은 URL로 접속하면 동일 사례가 보입니다.
(기본 공용 ID는 `kiwoom-team`. 개인/별도 공간이 필요하면 `?user=이름` 을 붙여 접속.)
